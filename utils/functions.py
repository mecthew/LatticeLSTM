...
def read_seg_instance(input_file, word_alphabet, biword_alphabet, char_alphabet, label_alphabet,
                      span_label_alphabet, attr_label_alphabet,
    ...
    chars = []
    labels = []
    span_labels, attr_start_labels, attr_end_labels = [], [], []
    word_Ids = []
    biword_Ids = []
    char_Ids = []
    label_Ids = []
    span_labels_Ids, attr_start_labels_Ids, attr_end_labels_Ids = [], [], []
    for idx in range(len(in_lines)):
        ...
        if len(line) > 2:
            ...
            labels.append(label)
            span_labels.append(label[0])
            attr_start_labels.append(label[2:] if label[0] in ['B', 'S'] else ATTR_NULLKEY)   # '' represent 'null'
            attr_end_labels.append(label[2:] if label[0] in ['E', 'S'] else ATTR_NULLKEY)
            word_Ids.append(word_alphabet.get_index(word))
            biword_Ids.append(biword_alphabet.get_index(biword))
            label_Ids.append(label_alphabet.get_index(label))
            span_labels_Ids.append(span_label_alphabet.get_index(span_labels[-1]))
            attr_start_labels_Ids.append(attr_label_alphabet.get_index(attr_start_labels[-1]))
            attr_end_labels_Ids.append(attr_label_alphabet.get_index(attr_end_labels[-1]))
            char_list = []
            char_Id = []
            for char in word:
                char_list.append(char)
            ...
            if (max_sent_length < 0) or (len(words) < max_sent_length):
                instence_texts.append([words, biwords, chars, labels, span_labels, attr_start_labels, attr_end_labels])
                instence_Ids.append([word_Ids, biword_Ids, char_Ids, label_Ids, span_labels_Ids, attr_start_labels_Ids, attr_end_labels_Ids])
            words = []
            biwords = []
            chars = []
            labels = []
            ...
            char_Ids = []
            label_Ids = []
            span_labels, span_labels_Ids = [], []
            attr_start_labels, attr_start_labels_Ids = [], []
            attr_end_labels, attr_end_labels_Ids = [], []
    return instence_texts, instence_Ids
    ...
    for idx in range(len(in_lines)):
        ...
        if len(line) > 2:
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words) > 0):
                ...
                instence_texts.append([words, biwords, chars, gazs, labels, span_labels, attr_start_labels, attr_end_labels])
                instence_Ids.append([word_Ids, biword_Ids, char_Ids, gaz_Ids, label_Ids, span_labels_Ids, attr_start_labels_Ids, attr_end_labels_Ids])
            words = []
            biwords = []
            chars = []
            labels = []
